...
title: Attributes for Graph Objects author: Seth Falcon clean: false
date: "`r format(Sys.time(), '%d %B, %Y')`" output: html_document: toc:
yes df_print: paged number_sections: yes ---

# Introduction

The *graph* package provides representations of graphs (nodes and edges)
...
work. See Figure [1](#fig1).

```{r exampleGraph1, warning=FALSE, message=FALSE}
library("graph")
...
```{r foo, fig.height=6, fig.width=6}
...
<a name="#fig1"></a>Figure 1: The graph `g1`
...
Attributes are set using the replacement form of `edgeData`. This method
allows the user to update the attribute for single edge, set the
attributes for a collection of edges to a single value, and to set the
attributes for a collection of edges to different values specified by a
vector of values.
...
It is also possible to set multiple attributes to different values in a
single call to `edgeData`.
...
Finally, we can set the an attribute to a vector of values by packing it
into a list:
